// SearchAgent.java: Agent who solves a problem by tree search. // It is task independent and supports both blind and heruistic search.
...
abstract class SearchAgent {
	...
	LinkedList solution;
	void setProblem(Problem p) { this.problem = p; }
	...
	void search() {
		...
		tree.add(root);
		insertFringe(root, fringe);// one copy of root is referenced in both
									// lists
		...
		while (fringe.size() != 0) {
			...
			LinkedList successors = problem.getSuccessor(nodeState); // child
																		// states
